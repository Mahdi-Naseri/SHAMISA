...
class LIVEDataset(SyntheticIQADataset):
    ...
    def __init__(
        ...
        self.images = np.array(
            [self.root / el.relative_to(el.parts[0]) for el in self.images]
        )
        ...
        self.ref_images = np.array(
            [self.root / el.relative_to(el.parts[0]) for el in self.ref_images]
        )

        self.mos = np.array(scores_csv["score"].values.tolist())

        self.distortion_types = scores_csv["dis_type"].values.tolist()
        self.distortion_types = np.array(
            [distortion_types_mapping[el] for el in self.distortion_types]
        )
        self.distortion_groups = np.array(
            [available_distortions[el] for el in self.distortion_types]
        )
