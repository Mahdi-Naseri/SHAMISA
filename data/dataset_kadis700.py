...
class KADIS700Dataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if random.random() > self.pristine_prob and self.max_distortions > 0:
            img_A_orig, distort_functions, distort_values = distort_images(
                img_A_orig,
                max_distortions=self.max_distortions,
                num_levels=self.num_levels,
            )
            img_A_ds, _, _ = distort_images(
                img_A_ds,
                distort_functions=distort_functions,
                distort_values=distort_values,
            )
            img_B_orig, _, _ = distort_images(
                img_B_orig,
                distort_functions=distort_functions,
                distort_values=distort_values,
            )
            img_B_ds, _, _ = distort_images(
                img_B_ds,
                distort_functions=distort_functions,
                distort_values=distort_values,
            )
        ...
        return {
            ...
            "distortion_functions": distort_functions,
