...
class TID2013Dataset(SyntheticIQADataset):
    ...
    def __init__(
        ...
        self.images = np.array(
            [self.root / "distorted_images" / el for el in self.images]
        )

        self.ref_images = np.array(
            [self.root / "reference_images" / el for el in self.ref_images]
        )
        self.ref_images = np.array(
            [
                el if el.exists() else self.root / "reference_images" / el.name.lower()
                for el in self.ref_images
            ]
        )
        ...
        if self.phase != "all":
            ...
            split_idxs = np.array(
                list(filter(lambda x: x != -1, split_idxs))
            )  # Remove the padding (i.e. -1 indexes)
...
distortion_groups = {
    "blur": ["gaussian_blur"],
    "color_distortion": [
        "intensive_color_noise",
        "color_saturation_change",
        "color_quantization_with_dither",
