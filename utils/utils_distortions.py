...
def fspecial(
    filter_type: str, p2: Union[int, Tuple[int, int]], p3: Union[int, float] = None
) -> np.ndarray:
    if filter_type == "gaussian":
        m, n = [(ss - 1.0) / 2.0 for ss in p2]
        y, x = np.ogrid[-m : m + 1, -n : n + 1]
        h = np.exp(-(x * x + y * y) / (2.0 * p3 * p3))
        ...
        sgrid = (
            ...
            * (
            ...
            np.logical_and(
                np.logical_and(minxy == 0, maxxy - 0.5 < rad), maxxy + 0.5 >= rad
            ),
        )
        ...
        lastpix = np.where(
            np.logical_and((rad >= half_len), (abs(dist2line) <= linewdt))
        )
        x2lastpix = half_len - np.abs(
...
def curves(xx: torch.Tensor, coef: float) -> torch.Tensor:
    if type(coef) == list:
        coef = [[0.3, 0.5, 0.7], [coef[0], 0.5, coef[1]]]
    else:
        coef = [[0.5], [coef]]
...
def spline(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    if n == 3:
        ...
        coefs = np.vstack(
            ((dzdxdx - dzzdx) / dxd, 2 * dzzdx - dzdxdx, s[0 : n - 1], y[0 : n - 1])
        ).T
...
def imscatter(x: torch.Tensor, amount: float, iterations=1) -> torch.Tensor:
    ...
    for i in range(iterations):
        ...
        xx, yy = torch.as_tensor(
            np.mgrid[0 : y.shape[1], 0 : y.shape[2]], device=x.device
        )
...
def bilinear_interpolate_torch(
    im: torch.Tensor, x: torch.Tensor, y: torch.Tensor, eps: float = 1e-8
) -> torch.Tensor:
