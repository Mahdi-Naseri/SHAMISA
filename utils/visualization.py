...
def visualize_tsne_umap_mos(
    ...
    for method in methods:
        ...
        for d_group, color in dist_color_maps.items():
            placeholder = features_embedded[
                np.where(dist_colors == f"rgba({color}, 1.0)")[0][0]
            ]  # Needed to create the legend correctly. Store the coordinates of a point with max opacity for each class
        ...
        if args["n_components"] == 2:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(size=5, color=dist_colors),
                    showlegend=False,
                )
            )
        elif args["n_components"] == 3:
        ...
        # Update layout
        fig.update_layout(
            title=f"{method} KADID10K visualization",
        ...
        for mos, opacity in mos_shades.items():
            placeholder = features_embedded[
                np.where(mos_colors == f"rgba({mos_color_map}, {opacity})")[0][0]
            ]  # Needed to create the legend correctly. Store the coordinates of a point for each class
            if args["n_components"] == 2:
                trace = go.Scatter(
                    x=[placeholder[0]],
                    y=[placeholder[1]],
        ...
        if args["n_components"] == 2:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=features_embedded[:, 1],
                    mode="markers",
                    marker=dict(size=5, color=mos_colors),
                    showlegend=False,
                )
            )
        elif args["n_components"] == 3:
        ...
        # Update layout
        fig.update_layout(
            title=f"{method} MOS KADID10K visualization",
            scene=dict(xaxis_title="X", yaxis_title="Y", zaxis_title="Z"),
            legend=dict(x=0, y=1, bgcolor="rgba(0,0,0,0)"),
        )
