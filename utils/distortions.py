...
def color_diffusion(x: torch.Tensor, amount: int) -> torch.Tensor:
    ...
    y = torch.trunc(kornia.color.lab_to_rgb(lab) * 255.0) / 255.0
    y = y[:, [2, 1, 0]].squeeze(0)
    return y
...
def color_shift(x: torch.Tensor, amount: int) -> torch.Tensor:
    ...
    y = F.pad(
        y.transpose(1, 0), (amount_shift[1], amount_shift[1]), mode="replicate"
    ).transpose(1, 0)
...
def color_saturation2(x: torch.Tensor, factor: int) -> torch.Tensor:
    ...
    y = torch.trunc(kornia.color.lab_to_rgb(lab) * 255) / 255.0
    return y[[2, 1, 0], ...]
...
def jpeg2000(x: torch.Tensor, ratio: int) -> torch.Tensor:
    ...
    source = x.clamp(0, 1)
    x_uint8 = (source * 255.0).byte().cpu().numpy()
    image = Image.fromarray(x_uint8.transpose(1, 2, 0), "RGB")
    with io.BytesIO() as output:
        try:
            image.save(output, format="JPEG2000", **compression_params)
            output.seek(0)
            with Image.open(output) as decoded:
                return transforms.ToTensor()(decoded.convert("RGB"))
        except OSError:
            with tempfile.NamedTemporaryFile(suffix=".jp2", delete=False) as handle:
                temp_path = handle.name

            try:
                image.save(temp_path, format="JPEG2000", **compression_params)
                with Image.open(temp_path) as decoded:
                    return transforms.ToTensor()(decoded.convert("RGB"))
            except OSError:
                approx_quality = max(5, min(95, int(round(100.0 / max(ratio, 1)))))
                return jpeg(source.clone(), approx_quality)
            finally:
                try:
                    os.remove(temp_path)
                except OSError:
...
def jpeg(x: torch.Tensor, quality: int) -> torch.Tensor:
    ...
    y = (decode_jpeg(y) / 255.0).to(torch.float32)
    return y


def white_noise(
    x: torch.Tensor, var: float, clip: bool = True, rounds: bool = False
) -> torch.Tensor:
    ...
    if clip and rounds:
        y = torch.clip((y * 255.0).round(), 0, 255) / 255.0
    elif clip:
        y = torch.clip(y, 0, 1)
    elif rounds:
        y = (y * 255.0).round() / 255.0
    return y
...
def impulse_noise(x: torch.Tensor, d: float, s_vs_p: float = 0.5) -> torch.Tensor:
    ...
    coords = np.concatenate(
        (
            np.random.randint(0, 3, (num_sp, 1)),
            np.random.randint(0, x.shape[1], (num_sp, 1)),
            np.random.randint(0, x.shape[2], (num_sp, 1)),
        ),
        1,
    )
    ...
    coords_salt = torch.tensor(coords_salt, dtype=torch.long)
    coords_pepper = torch.tensor(coords_pepper, dtype=torch.long)

    # x[coords_salt] = 1
    # x[coords_pepper] = 0
    x[coords_salt[0], coords_salt[1], coords_salt[2]] = 1
    x[coords_pepper[0], coords_pepper[1], coords_pepper[2]] = 1
...
def non_eccentricity_patch(x: torch.Tensor, pnum: int) -> torch.Tensor:
    ...
    for i in range(pnum):
        ...
        patch = y[
            :, h_start : h_start + patch_size[0], w_start : w_start + patch_size[0]
        ]
        ...
        y[
            :,
            rand_h_start : rand_h_start + patch_size[0],
            rand_w_start : rand_w_start + patch_size[0],
        ] = patch
...
def quantization(x: torch.Tensor, levels: int) -> torch.Tensor:
    ...
    image = torch.bucketize(x.contiguous() * 255.0, bins).to(torch.float32)
    image = mapmm(image)
    return image
...
def non_linear_contrast_change(
    x: torch.Tensor,
    output_offset_value: float,
    output_central_value: float = 0.5,
    input_offset_value: float = 0.5,
    input_central_value: float = 0.5,
) -> torch.Tensor:
