...
class SimCLR(torch.nn.Module):
    ...
    def __init__(
        ...
        use_norm = getattr(encoder_params, "use_norm", None)
        if use_norm is None:
            use_norm_rep = getattr(encoder_params, "use_norm_rep", True)
            use_norm_emb = getattr(encoder_params, "use_norm_emb", True)
        else:
        ...
        self.encoder = ResNet(
            ...
            use_norm_rep=use_norm_rep,
            use_norm_emb=use_norm_emb,
        )

        self.temperature = temperature
        self.criterion = nt_xent_loss

    def forward(self, im_q, im_k=None, *args):
...
def nt_xent_loss(a: torch.Tensor, b: torch.Tensor, tau: float = 0.1):
    ...
    numerators = torch.exp(
        torch.div(torch.nn.CosineSimilarity()(a_cap_b_cap, b_cap_a_cap), tau)
    )
